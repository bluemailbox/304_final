...
$cid = $_GET['cid'];
$sql2 = "SELECT customerId, firstName, lastName, email, phonenum, C.address a, city, C.state s, postalCode, country, userid FROM customer C WHERE customerId = ?";
$pstmt2 = sqlsrv_query($con, $sql2, array($cid));
...
$country = $rst['country'];
?>
<form method="post" action="editcust.php" enctype="multipart/form-data"> 
...
function editCust() {
    ...
    $sql = "UPDATE Customer SET firstName=?, lastName=?, email=?, phoneNum=?, address=?, city=?, state=?, postalCode=?, country=?, password=? WHERE customerId =?";//update cust info
    include 'include/db_credentials.php';
    $con = sqlsrv_connect($server, $connectionInfo);
    ...
    $pstmt = sqlsrv_prepare($con, $sql, array($n_fname, $n_lname, $n_email, $n_pnum, $n_addr, $n_city, $n_state, $n_postc, $n_country, $n_pass, $custid));//
    sqlsrv_execute($pstmt); //execute sql insert
    sqlsrv_close($con);//close connection

    echo("<h3>Information has been updated for customer ID: ". $custid . "</h3>" ); //confirms item is added
}
?>
